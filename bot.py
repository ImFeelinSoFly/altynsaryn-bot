...
def query_with_fetchone(select, client_id):
    ...
    elif select == 'flag_for_full':
        query = "SELECT flag_for_full FROM user WHERE client_id = %s"
    elif select == 'menu_id':
        query = "SELECT menu_id FROM user WHERE client_id = %s"
    args = (client_id,)
    try:
...
def update_menu_id(menu_id, client_id):
    query = "update `testshit`.`user` \
    SET menu_id = %s\
    WHERE client_id = %s\
    "
    args = (menu_id, client_id)
...
def update_flag_for_full(flag_for_full, client_id):
    query = "update `testshit`.`user` \
    SET flag_for_full = %s\
    WHERE client_id = %s\
    "
    args = (flag_for_full, client_id)
...
"
    args = (user_id, 0, '', 0, 0, 0, 0, 0, 0, 0, message_id, user_name, message_id + 3, 0)
...
Cursor.execute('''CREATE TABLE IF NOT EXISTS user (id int NOT NULL auto_increment primary key,
    ...
    flag_for_full TINYINT UNSIGNED NOT NULL DEFAULT 0)''')
mydb.commit()
...
def full_school(call):
    ...
    but_2 = types.InlineKeyboardButton(text='Отмена', callback_data='delete_this_message' + 'full')
    fs.add(but_1)
    fs.add(but_2)
    return fs
...
def all_courses():
    ...
    but_1 = types.InlineKeyboardButton(text='Получить', callback_data='all')
    ac.add(but_1)
    return ac
...
def callbacks(call):
    ...
    try:
        ...
        with open('stats.txt', mode='w', encoding="utf-8") as file:
            file.writelines(content)
        pass
    if 'delete_this_message' in call.data:
        try:
            bot.delete_message(call.from_user.id, call.message.message_id)
        except:
            pass
        if 'full' in call.data:
            update_flag_for_full(0, call.from_user.id)
    elif 'suc' in call.data:
        try:
            ...
            bot.send_message(909435473,
                             text='Заказ под номером  ' + chat_id + ' подтвержден от @' + query_with_fetchone('user',
                                                                                                              chat_id))
            bot.delete_message(chat_id, query_with_fetchone('previous_message_id', chat_id) + 2)
            update_menu_id(query_with_fetchone('previous_message_id', chat_id) + 2, chat_id)
        ...
        if not query_with_fetchone('flag_for_full', call.from_user.id):
            update_flag_for_full(1, call.from_user.id)
            if 'WebForMySelf' in call.data:
                bot.send_photo(call.from_user.id,
                               photo='AgACAgIAAxkBAAICn18m-FpBRzuO5ixHkmQ20VafeJpkAAKPsDEbNyw5SYUAAVCTvWj-4VR165IuAAMBAAMCAANtAANGlQQAARoE',
                               caption='<b>15 курсов от топового онлайн-университета</b>\n\n🤢Цена курса:   ̶1̶6̶9̶ ̶0̶1̶0̶₽̶ ̶/̶ ̶9̶5̶9̶ ̶9̶9̶0̶₸̶\n🤑Наша цена: 1 499₽ / 7 990₸ ',
                               reply_markup=full_school(call.data), parse_mode='html')
            elif 'OTUS' in call.data:
                bot.send_photo(call.from_user.id,
                               photo='AgACAgIAAxkBAAICnl8m-FrlUqepVmYm1ZeDHoJJIxsCAAKOsDEbNyw5SZh15gZf6hirPdbGki4AAwEAAwIAA20AAzMxBQABGgQ',
                               caption='<b>15 курсов от топового онлайн-университета</b>\n\n🤢Цена курса:   ̶8̶3̶0̶ ̶3̶0̶0̶₽̶ ̶/̶ ̶4̶ ̶7̶1̶3̶ ̶9̶9̶0̶₸̶\n🤑Наша цена: 1 499₽ / 7 990₸ ',
                               reply_markup=full_school(call.data), parse_mode='html')
            elif 'SkillBox' in call.data:
                bot.send_photo(call.from_user.id,
                               photo='AgACAgIAAxkBAAICoV8m-Fog71e36wushxdoHzbj8sY2AAKRsDEbNyw5SfjruuihWSA1rFoVlS4AAwEAAwIAA20AA_VcAgABGgQ',
                               caption='<b>15 курсов от топового онлайн-университета</b>\n\n🤢Цена курса:   ̶8̶2̶4̶ ̶0̶0̶0̶₽̶ ̶/̶ ̶4̶ ̶6̶8̶0̶ ̶9̶9̶0̶₸̶\n🤑Наша цена: 1 499₽ / 7 990₸ ',
                               reply_markup=full_school(call.data), parse_mode='html')
            elif 'Udemy' in call.data:
                bot.send_photo(call.from_user.id,
                               photo='AgACAgIAAxkBAAICoF8m-FpfdYth6qqNKBAqjMsn1c9EAAKQsDEbNyw5Sby-cYSselILWEXCki4AAwEAAwIAA20AA-85BQABGgQ',
                               caption='<b>15 курсов от топового онлайн-университета</b>\n\n🤢Цена курса:   ̶8̶5̶ ̶7̶0̶0̶₽̶ ̶/̶ ̶4̶8̶6̶ ̶5̶9̶0̶₸̶\n🤑Наша цена: 1 499₽ / 7 990₸ ',
                               reply_markup=full_school(call.data), parse_mode='html')
            elif 'Netology' in call.data:
                bot.send_photo(call.from_user.id,
                               photo='AgACAgIAAxkBAAICol8m-FpYR9uD0aEHp-wuOPycPyREAAKSsDEbNyw5SYjzxETrjAAB92oLIJUuAAMBAAMCAANtAANXYAIAARoE',
                               caption='<b>15 курсов от топового онлайн-университета</b>\n\n🤢Цена курса:   ̶5̶8̶2̶ ̶1̶8̶0̶₽̶ ̶/̶ ̶3̶ ̶3̶0̶6̶ ̶9̶9̶0̶₸̶\n🤑Наша цена: 1 499₽ / 7 990₸ ',
                               reply_markup=full_school(call.data), parse_mode='html')
            elif 'GeekBrains' in call.data:
                bot.send_photo(call.from_user.id,
                               photo='AgACAgIAAxkBAAICnV8m-FoSW7W4fy5pfHV_CEy_qj3yAAKNsDEbNyw5SSors3SoEOle9bDvkS4AAwEAAwIAA20AAw0gBQABGgQ',
        ...
        if '16' in call.data:
            if 'card' == call.data[-4:]:
                bot.edit_message_text(chat_id=call.from_user.id, message_id=call.message.message_id,
                                      text='Вы выбрали оплату банковской картой:\nОплатить картой Kaspi bank\'а можно только через перевод',
                                      reply_markup=credit_keyboard(
                                          'https://qiwi.com/payment/form/99999?extra%5B%27account%27%5D=ALDARKOSE&amountInteger=1499&amountFraction=0&currency=643&blocked[0]=account&extra%5B%27accountType%27%5D=nickname&blocked[1]=sum',
                                          call.from_user.id))
            ...
            if 'card' == call.data[-4:]:
                bot.edit_message_text(chat_id=call.from_user.id, message_id=call.message.message_id,
                                      text='Вы выбрали оплату банковской картой:\nОплатить картой Kaspi bank\'а можно только через перевод',
                                      reply_markup=credit_keyboard(
                                          'https://qiwi.com/payment/form/99999?extra%5B%27account%27%5D=ALDARKOSE&amountInteger=1799&amountFraction=0&currency=643&blocked[0]=account&extra%5B%27accountType%27%5D=nickname&blocked[1]=sum',
                                          call.from_user.id))
            elif 'wallet' == call.data[-6:]:
                bot.edit_message_text(chat_id=call.from_user.id, message_id=call.message.message_id,
                                      text='Вы выбрали оплату через Qiwi кошелек:\n',
                ...
                bot.edit_message_text(chat_id=call.from_user.id, message_id=call.message.message_id,
                                      text='Вы выбрали оплату через Kaspi Gold\nПереведите 9 990₸ / 1 799₽ на карту: \n            5169 4971 7975 8408',
                                      reply_markup=credit_keyboard('https://kaspi.kz/Transfers/Landing/g2g',
                                                                   call.from_user.id))
            ...
            if 'card' == call.data[-4:]:
                bot.edit_message_text(chat_id=call.from_user.id, message_id=call.message.message_id,
                                      text='Вы выбрали оплату банковской картой:\nОплатить картой Kaspi bank\'а можно только через перевод',
                                      reply_markup=credit_keyboard(
                                          'https://qiwi.com/payment/form/99999?extra%5B%27account%27%5D=ALDARKOSE&amountInteger=899&amountFraction=0&currency=643&blocked[0]=account&extra%5B%27accountType%27%5D=nickname&blocked[1]=sum',
                                          call.from_user.id))
            elif 'wallet' == call.data[-6:]:
                bot.edit_message_text(chat_id=call.from_user.id, message_id=call.message.message_id,
                                      text='Вы выбрали оплату через Qiwi кошелек:\n',
                ...
                bot.edit_message_text(chat_id=call.from_user.id, message_id=call.message.message_id,
                                      text='Вы выбрали оплату через Kaspi Gold\nПереведите 4 990₸ / 899₽ на карту: \n              5169 4971 7975 8408',
                                      reply_markup=credit_keyboard('https://kaspi.kz/Transfers/Landing/g2g',
                                                                   call.from_user.id))
    elif call.data == 'confirmed':
        update_previous_message_id(call.message.message_id, call.from_user.id)
        try:
            for i in range(query_with_fetchone('menu_id', call.from_user.id), call.message.message_id):
                try:
        ...
        if not query_with_fetchone("flag_copyright", call.from_user.id):
            ...
            bot.send_document(chat_id=call.from_user.id,
                              caption='Если вы обладатель курса, то просим ознакомиться вас с этим документом:\n',
                              data='BQACAgIAAxkBAAIECl8ohaQ5gYopMfi8ue4DwCkjhUPPAAL_CgACEjlBSWSU3n3pQ-eVGgQ')
...
def startpg(message):
    ...
    main_keyboard.row('Все 90 курсов за 9 990₸ / 1 799₽')
    ...
    update_flag_for_full(0, message.from_user.id)
    update_menu_id(message.message_id + 3, message.from_user.id)
...
def essential(message):
    ...
    try:
        ...
        update_previous_message_id(message.message_id, message.from_user.id)
    if message.text == 'Все 90 курсов за 9 990₸ / 1 799₽':
        bot.send_photo(message.from_user.id,
                       photo='AgACAgIAAxkBAAIFkl9GXt5Ko-SkmsuK0rxNUdtbWrUWAAIOrzEbF2Y5SndGWRgG0NoIxXQblS4AAwEAAwIAA20AAx1yAwABGwQ',
                       caption='<b>Пакет курсов "От 0 до лучшего в СНГ"</b>\n\n✅Все актуальные языки программирования \n✅Все сферы и направления в программировании\n✅Поймёте, что подходит именно ВАМ\n✅Впитаете знания лучших программистов СНГ\n✅Получите как минимум одну стабильную профессию\n\n<i>6 ОНЛАЙН-УНИВЕРСИТЕТОВ, 90 ВИДЕОКУРСОВ ПО IT-ПРОФЕССИЯМ</i>\n\n🤢Цена курса:  ̶3̶ ̶3̶5̶3̶ ̶0̶7̶0̶₽̶ /\n                                ̶1̶9̶ ̶3̶6̶8̶ ̶0̶0̶0̶₸̶\n🤑Наша цена: 1 799₽ / 9 990₸',
                       reply_markup=all_courses(), parse_mode='html')
